	...
	<main role="main" class="container-fluid">
		<div class="row">
			...
			<div class="col-12 col-sm-12 col-md-12 col-lg-9 col-xl-9 order-1 order-sm-1 order-md-1 order-lg-2 order-xl-2 pl-0 pr-0 bg-white banner">
				@if (!empty($product->banner->focus_point))
					<div class="focuspoint" data-focus-x="{{ $product->banner->focus_point->focusX }}" data-focus-y="{{ $product->banner->focus_point->focusY }}">
						<img src="{{ $product->banner->url }}" alt="{{ $product->title }}" class="img-fluid">
						<div class="position-absolute d-flex h-100 align-items-center justify-content-center banner-title">
							<h3 class="font-vtg-stencil display-5 text-uppercase text-white text-center">{{-- $product->title --}}<small class="d-block">{{ $contentType->title }}</small></h3>
						</div>
					</div>
				@elseif (!empty($product->image->focus_point))
													...
													<div class="col-12 col-sm-6 col-md-3 col-lg-3 col-xl-3">
														<div class="card text-center mb-4">
															...
															@else
																...
																@else
																	<a href="{{ $tempUrl }}" title="{{ $supportingFile->name }}" target="_blank" class="text-dark pt-3"><i class="fa fa-file fa-5x align-middle" aria-hidden="true"></i><br><br>No Preview Available</a>
																@endif
															@endif
															<div class="card-body">
