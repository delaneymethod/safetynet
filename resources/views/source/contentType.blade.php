	...
	<main role="main" class="container-fluid">
		<div class="row">
			...
			<div class="col-12 col-sm-12 col-md-12 col-lg-9 col-xl-9 order-1 order-sm-1 order-md-1 order-lg-2 order-xl-2 pl-0 pr-0 bg-white banner">
				@if (!empty($contentType->banner->focus_point))
					<div class="focuspoint" data-focus-x="{{ $contentType->banner->focus_point->focusX }}" data-focus-y="{{ $contentType->banner->focus_point->focusY }}">
						<img src="{{ $contentType->banner->url }}" alt="{{ $contentType->title }}" class="img-fluid">
						<div class="position-absolute d-flex h-100 align-items-center justify-content-center banner-title">
							<h3 class="font-vtg-stencil display-5 text-uppercase text-white text-center">{{ $category->title }}<small class="d-block">{{ $contentType->title }}</small></h3>
						</div>
					</div>
				@elseif (!empty($contentType->image->focus_point))
