...
@section('title', 'Edit Sector - Sectors - '.config('app.name'))
@section('description', 'Edit Sector - Sectors - '.config('app.name'))
@section('keywords', 'Edit, Sector, Sectors, '.config('app.name'))
...
@section('formButtons')
	<div class="form-buttons">
		...
		@endif
		<button type="submit" name="submit_edit_sector" id="submit_edit_sector" class="pull-right btn btn-primary" title="Save Changes"><i class="icon fa fa-check-circle" aria-hidden="true"></i>Save Changes</button>
	</div>
@endsection
...
@section('content')
		...
		<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
			...
			<div class="content padding bg-white">
				<form name="editSector" id="editSector" class="editSector" role="form" method="POST" action="/cp/sectors/{{ $sector->id }}">
					{{ csrf_field() }}
					{{ method_field('PUT') }}
					...
					<div class="form-group">
						...
						<input type="text" name="title" id="title" class="form-control" value="{{ old('title', optional($sector)->title) }}" placeholder="e.g Test Title" tabindex="1" autocomplete="off" aria-describedby="helpBlockTitle" required autofocus>
						@if ($errors->has('title'))
					...
					<div class="form-group">
						...
						<input type="text" name="slug" id="slug" class="form-control" value="{{ old('slug', optional($sector)->slug) }}" placeholder="e.g Test Slug" tabindex="2" autocomplete="off" aria-describedby="helpBlockSlug" required>
						@if ($errors->has('slug'))
					...
					<div class="form-group">
						...
						<textarea name="description" id="description" class="form-control" autocomplete="off" placeholder="e.g Test Description" rows="5" cols="50" tabindex="3" aria-describedby="helpBlockDescription" required>{{ old('description', optional($sector)->description) }}</textarea>
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="text" name="banner" id="banner" class="form-control" autocomplete="off" placeholder="e.g Test Banner Image" value="{{ old('banner', optional($sector)->banner->path) }}" tabindex="4" autocomplete="off" aria-describedby="helpBlockBanner">
							<div class="input-group-append">
								<a href="javascript:void(0);" title="Select Asset" rel="nofollow" data-toggle="modal" data-target="#banner-browse-modal" data-field_id="banner" data-value="{{ old('banner', optional($sector)->banner->url) }}" class="btn btn-secondary">Select Asset</a>
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="text" name="image" id="image" class="form-control" autocomplete="off" placeholder="e.g Test Image" value="{{ old('image', optional($sector)->image->path) }}" tabindex="5" autocomplete="off" aria-describedby="helpBlockImage" required>
							<div class="input-group-append">
								<a href="javascript:void(0);" title="Select Asset" rel="nofollow" data-toggle="modal" data-target="#image-browse-modal" data-field_id="image" data-value="{{ old('image', optional($sector)->image->url) }}" class="btn btn-secondary">Select Asset</a>
					...
					<div class="form-group">
						...
						<input type="text" name="yammer" id="yammer" class="form-control" value="{{ old('yammer', optional($sector)->yammer) }}" placeholder="e.g 13172895" tabindex="6" autocomplete="off" aria-describedby="helpBlockYammer">
					...
					<div class="form-group">
						...
						<input type="text" name="stream" id="stream" class="form-control" value="{{ old('stream', optional($sector)->stream) }}" placeholder="e.g 30ae7639-646f-45ff-bee1-9ff172682c0b" tabindex="7" autocomplete="off" aria-describedby="helpBlockStream">
					...
					<div class="form-group">
						...
						<input type="color" name="colour" id="colour" value="{{ old('colour', ($sector->colour) ? $sector->colour : '#fe5001') }}" placeholder="e.g Red" tabindex="8" autocomplete="off" aria-describedby="helpBlockColour">
						...
						<div class="custom-controls-stacked">
							@foreach ($statuses as $status)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start status_id-{{ $status->id }}">
									<input type="radio" name="status_id" id="status_id-{{ str_slug($status->title) }}" class="custom-control-input" value="{{ $status->id }}" tabindex="9" aria-describedby="helpBlockStatusId" {{ ($sector->status_id == $status->id || old('status_id') == $status->id) ? 'checked' : '' }}>
					...
					<div class="form-group">
						...
						@else
							@php ($departmentIds = $sector->departments->pluck('id')->toArray())
						@endif
