		...
		<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
			...
			<div class="content padding bg-white">
				<form name="createUser" id="createUser" class="createUser" role="form" method="POST" action="/cp/users">
					...
					<p><i class="fa fa-info-circle" aria-hidden="true"></i> Fields marked with <span class="text-danger">&#42;</span> are required.<br><i class="fa fa-info-circle" aria-hidden="true"></i> New users will receive an email to set up a password.</p>
					<div class="spacer"></div>
					...
					<div class="form-group">
						...
						<input type="tel" name="telephone" id="telephone" class="form-control" value="{{ old('telephone') }}" placeholder="e.g +44 1224 123 4567" tabindex="4" autocomplete="off" aria-describedby="helpBlockTelephone">
					...
					<div class="form-group">
						...
						<input type="text" name="job_title" id="job_title" class="form-control" value="{{ old('job_title') }}" placeholder="e.g Senior Design Engineer" tabindex="5" autocomplete="off" aria-describedby="helpBlockJobTitle">
					...
					<div class="form-group">
						...
						<input type="text" name="skype" id="skype" class="form-control" value="{{ old('skype') }}" placeholder="e.g echo123" tabindex="6" autocomplete="off" aria-describedby="helpBlockSkype">
						@if ($errors->has('skype'))
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="text" name="image" id="image" class="form-control" autocomplete="off" placeholder="e.g Test Mugshot" value="{{ old('image') }}" tabindex="7" autocomplete="off" aria-describedby="helpBlockMugshot">
					...
					<div class="form-group">
						...
						<textarea name="bio" id="bio" class="form-control" autocomplete="off" placeholder="e.g Test Bio" rows="5" cols="50" tabindex="8" aria-describedby="helpBlockBio">{{ old('bio') }}</textarea>
						@if ($errors->has('bio'))
						...
						<div class="custom-controls-stacked">
							@foreach ($locations as $location)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start">
									<input type="radio" name="location_id" id="location_id-{{ $location->id }}" class="custom-control-input" value="{{ $location->id }}" tabindex="9" aria-describedby="helpBlockLocationId" {{ (old('location_id') == $location->id) ? 'checked' : ($loop->last) ? 'checked' : '' }}>
					...
					<div class="form-group">
						...
						<div class="custom-controls-stacked">
							@foreach ($roles as $role)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start">
									<input type="radio" name="role_id" id="role_id-{{ $role->id }}" class="custom-control-input" value="{{ $role->id }}" tabindex="10" aria-describedby="helpBlockRoleId" {{ (old('role_id') == $role->id) ? 'checked' : ($loop->last) ? 'checked' : '' }}>
						...
						<div class="custom-controls-stacked">
							@foreach ($statuses as $status)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start status_id-{{ $status->id }}">
									<input type="radio" name="status_id" id="status_id-{{ str_slug($status->title) }}" class="custom-control-input" value="{{ $status->id }}" tabindex="11" aria-describedby="helpBlockStatusId" {{ (old('status_id') == $status->id) ? 'checked' : ($loop->first) ? 'checked' : '' }}>
