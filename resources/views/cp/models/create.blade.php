...
@section('title', 'Create Model - Models - '.config('app.name'))
@section('description', 'Create Model - Models - '.config('app.name'))
@section('keywords', 'Create, Model, Models, '.config('app.name'))
...
@section('formButtons')
	<div class="form-buttons">
		...
		@endif
		<button type="submit" name="submit_create_model" id="submit_create_model" class="pull-right btn btn-primary" title="Save Changes"><i class="icon fa fa-check-circle" aria-hidden="true"></i>Save Changes</button>
	</div>
@endsection
...
@section('content')
		...
		<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
			...
			<div class="content padding bg-white">
				<form name="createModel" id="createModel" class="createModel" role="form" method="POST" action="/cp/models">
					{{ csrf_field() }}
					@yield('formButtons')
					...
					<div class="form-group">
						...
						<input type="number" name="minimum_number_of_units" id="minimum_number_of_units" class="form-control" value="{{ old('minimum_number_of_units') }}" placeholder="e.g 10" tabindex="3" autocomplete="off" aria-describedby="helpBlockMinimumNumberOfUnits" min="0" max="1000" required>
						...
						<div class="custom-controls-stacked">
							@foreach ($statuses as $status)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start status_id-{{ $status->id }}">
									<input type="radio" name="status_id" id="status_id-{{ str_slug($status->title) }}" class="custom-control-input" value="{{ $status->id }}" tabindex="4" aria-describedby="helpBlockStatusId" {{ (old('status_id') == $status->id) ? 'checked' : ($loop->first) ? 'checked' : '' }}>
