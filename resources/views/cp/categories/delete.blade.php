...
@section('content')
		...
		<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
			...
			<div class="content padding bg-white">
				<p class="text-center text-sm-center text-md-left text-lg-left text-xl-left">Please confirm that you wish to delete the <strong>{{ $category->title }}</strong> category.</p>
				...
				<form name="removeCategory" id="removeCategory" class="removeCategory" role="form" method="POST" action="/cp/categories/{{ $category->id }}">
					...
					<div class="form-buttons">
						@if ($currentUser->hasPermission('view_categories'))
							<a href="/cp/categories" title="Cancel" class="btn btn-link text-secondary" tabindex="2" title="Cancel">Cancel</a>
						@endif
						<button type="submit" name="submit_remove_category" id="submit_remove_category" class="pull-right btn btn-danger" tabindex="1" title="Delete">Delete</button>
					</div>
				</form>
			</div>
