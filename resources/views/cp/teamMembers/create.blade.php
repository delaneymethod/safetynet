...
@section('title', 'Create Team Member - Team Members - '.config('app.name'))
@section('description', 'Create Team Member - Team Members - '.config('app.name'))
@section('keywords', 'Create, Team Member, Team Members, '.config('app.name'))
...
@section('formButtons')
	<div class="form-buttons">
		...
		@endif
		<button type="submit" name="submit_create_team_member" id="submit_create_team_member" class="pull-right btn btn-primary" tabindex="11" title="Save Changes"><i class="icon fa fa-check-circle" aria-hidden="true"></i>Save Changes</button>
	</div>
@endsection
...
@section('content')
		...
		<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
			...
			<div class="content padding bg-white">
				<form name="createTeamMember" id="createTeamMember" class="createTeamMember" role="form" method="POST" action="/cp/team-members">
					{{ csrf_field() }}
					@yield('formButtons')
					...
					<div class="form-group">
						...
						<input type="text" name="full_name" id="full_name" class="form-control" value="{{ old('full_name') }}" placeholder="e.g Joe" tabindex="1" autocomplete="off" aria-describedby="helpBlockFullName" required autofocus>
					...
					<div class="form-group">
						...
						<input type="email" name="email" id="email" class="form-control" value="{{ old('email') }}" placeholder="e.g joe@bloggs.com" tabindex="2" autocomplete="off" aria-describedby="helpBlockEmailAddress" required>
						@if ($errors->has('email'))
					...
					<div class="form-group">
						...
						<input type="text" name="job_title" id="job_title" class="form-control" value="{{ old('job_title') }}" placeholder="e.g Senior Design Engineer" tabindex="3" autocomplete="off" aria-describedby="helpBlockJobTitle">
					...
					<div class="form-group">
						...
						<textarea name="bio" id="bio" class="form-control" autocomplete="off" placeholder="e.g Test Bio" rows="5" cols="50" tabindex="5" aria-describedby="helpBlockBio">{{ old('bio') }}</textarea>
						@if ($errors->has('bio'))
						...
						<div class="custom-controls-stacked">
							@foreach ($locations as $location)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start">
									<input type="radio" name="location_id" id="location_id-{{ $location->id }}" class="custom-control-input" value="{{ $location->id }}" tabindex="6" aria-describedby="helpBlockLocationId" {{ (old('location_id') == $location->id) ? 'checked' : ($loop->last) ? 'checked' : '' }}>
