...
@section('title', 'Create Event - Events - '.config('app.name'))
@section('description', 'Create Event - Events - '.config('app.name'))
@section('keywords', 'Create, Event, Events, '.config('app.name'))
...
@section('formButtons')
	<div class="form-buttons">
		...
		@endif
		<button type="submit" name="submit_create_event" id="submit_create_event" class="pull-right btn btn-primary" title="Save Changes"><i class="icon fa fa-check-circle" aria-hidden="true"></i>Save Changes</button>
	</div>
@endsection
...
@section('content')
		...
		<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
			...
			<div class="content padding bg-white">
				<form name="createEvent" id="createEvent" class="createEvent" role="form" method="POST" action="/cp/events">
					{{ csrf_field() }}
					@yield('formButtons')
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="text" name="image" id="image" class="form-control" autocomplete="off" placeholder="e.g Test Image" value="{{ old('image') }}" tabindex="4" autocomplete="off" aria-describedby="helpBlockImage" required>
						...
						<div class="custom-controls-stacked">
							@foreach ($allDayOptions as $allDayOption)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start all_day-{{ $allDayOption->value }}">
									<input type="radio" name="all_day" id="all_day-{{ str_slug($allDayOption->title) }}" class="custom-control-input" value="{{ $allDayOption->value }}" tabindex="5" aria-describedby="helpBlockAllDay" {{ (old('all_day') == $allDayOption->value) ? 'checked' : ($loop->first) ? 'checked' : '' }}>
					...
					<div class="form-group">
						<div class="row">
							<div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
								...
								<input type="hidden" name="start" id="start" value="{{ old('start', date('Y-m-d H:i')) }}" tabindex="6" aria-describedby="helpBlockStart">
							...
							<div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
								...
								<input type="hidden" name="end" id="end" value="{{ old('end', date('Y-m-d H:i', strtotime('+1 hour'))) }}" tabindex="7" aria-describedby="helpBlockEnd">
							...
							<div class="multi-field">
								<div class="row">
									<div class="col-4">
										...
										<input type="text" name="starts[]" value="{{ old('start', date('Y-m-d H:i')) }}" class="form-control" placeholder="30-06-2018 09:00" autocomplete="off" required>
									</div>
									<div class="col-4">
										<label for="ends" class="control-label font-weight-bold">End Date / Time <span class="text-danger">&#42;</span></label>
										<input type="text" name="ends[]" value="{{ old('end', date('Y-m-d H:i', strtotime('+1 hour'))) }}" class="form-control" placeholder="30-06-2018 17:00" autocomplete="off" required>
									</div>
									<div class="col-1">
										<label>&nbsp;</label>
					...
					<div class="form-group">
						...
						<input type="url" name="url" id="url" class="form-control" value="{{ old('url') }}" placeholder="e.g http://www." tabindex="8" autocomplete="off" aria-describedby="helpBlockUrl">
						@if ($errors->has('url'))
					...
					<div class="form-group">
						...
						<input type="text" name="organiser_name" id="organiser_name" class="form-control" value="{{ old('organiser_name') }}" placeholder="e.g " tabindex="9" autocomplete="off" aria-describedby="helpBlockOrganiserName">
					...
					<div class="form-group">
						...
						<input type="text" name="organiser_email" id="organiser_email" class="form-control" value="{{ old('organiser_email') }}" placeholder="e.g " tabindex="10" autocomplete="off" aria-describedby="helpBlockOrganiserEmail">
					...
					<div class="form-group">
						...
						<input type="text" name="organiser_contact_number" id="organiser_contact_number" class="form-control" value="{{ old('organiser_contact_number') }}" placeholder="e.g " tabindex="11" autocomplete="off" aria-describedby="helpBlockOrganiserContactNumber">
						...
						<div class="custom-controls-stacked">
							@foreach ($statuses as $status)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start status_id-{{ $status->id }}">
									<input type="radio" name="status_id" id="status_id-{{ str_slug($status->title) }}" class="custom-control-input" value="{{ $status->id }}" tabindex="12" aria-describedby="helpBlockStatusId" {{ (old('status_id') == $status->id) ? 'checked' : ($loop->first) ? 'checked' : '' }}>
