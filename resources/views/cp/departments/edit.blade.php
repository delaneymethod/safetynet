...
@section('title', 'Edit Department - Departments - '.config('app.name'))
@section('description', 'Edit Department - Departments - '.config('app.name'))
@section('keywords', 'Edit, Department, Departments, '.config('app.name'))
...
@section('formButtons')
	<div class="form-buttons">
		@if ($currentUser->hasPermission('view_departments'))
			<a href="/cp/departments" title="Cancel" class="btn btn-link text-secondary" title="Cancel">Cancel</a>
		@endif
		<button type="submit" name="submit_edit_department" id="submit_edit_department" class="pull-right btn btn-primary" title="Save Changes"><i class="icon fa fa-check-circle" aria-hidden="true"></i>Save Changes</button>
	</div>
@endsection
...
@section('content')
		...
		<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
			...
			<div class="content padding bg-white">
				<form name="editDepartment" id="editDepartment" class="editDepartment" role="form" method="POST" action="/cp/departments/{{ $department->id }}">
					{{ csrf_field() }}
					{{ method_field('PUT') }}
					...
					<div class="form-group">
						...
						<input type="text" name="title" id="title" class="form-control" value="{{ old('title', optional($department)->title) }}" placeholder="e.g Test Department" tabindex="1" autocomplete="off" aria-describedby="helpBlockTitle" required autofocus>
						@if ($errors->has('title'))
					...
					<div class="form-group">
						...
						<input type="text" name="slug" id="slug" class="form-control" value="{{ old('slug', optional($department)->slug) }}" placeholder="e.g Test Slug" tabindex="2" autocomplete="off" aria-describedby="helpBlockSlug" required>
						@if ($errors->has('slug'))
					...
					<div class="form-group">
						...
						<textarea name="description" id="description" class="form-control" autocomplete="off" placeholder="e.g Test Description" rows="5" cols="50" tabindex="3" aria-describedby="helpBlockDescription" required>{{ old('description', optional($department)->description) }}</textarea>
					...
					<div class="spacer"></div>
					<div class="form-group">
						<label for="banner" class="control-label font-weight-bold">Banner <span class="text-danger">&#42;</span></label>
						<div class="input-group">
							<input type="text" name="banner" id="banner" class="form-control" autocomplete="off" placeholder="e.g Test Banner Image" value="{{ old('banner', optional($department)->banner->path) }}" tabindex="4" autocomplete="off" aria-describedby="helpBlockBanner" required>
							<div class="input-group-append">
								<a href="javascript:void(0);" title="Select Asset" rel="nofollow" data-toggle="modal" data-target="#banner-browse-modal" data-field_id="banner" data-value="{{ old('banner', optional($department)->banner->url) }}" class="btn btn-secondary">Select Asset</a>
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="text" name="image" id="image" class="form-control" autocomplete="off" placeholder="e.g Test Department Image" value="{{ old('image', optional($department)->image->path) }}" tabindex="5" autocomplete="off" aria-describedby="helpBlockImage" required>
							<div class="input-group-append">
								<a href="javascript:void(0);" title="Select Asset" rel="nofollow" data-toggle="modal" data-target="#image-browse-modal" data-field_id="image" data-value="{{ old('image', optional($department)->image->url) }}" class="btn btn-secondary">Select Asset</a>
					...
					<div class="form-group">
						...
						<input type="text" name="yammer" id="yammer" class="form-control" value="{{ old('yammer', optional($department)->yammer) }}" placeholder="e.g 13172895" tabindex="6" autocomplete="off" aria-describedby="helpBlockYammer">
					...
					<div class="form-group">
						...
						<input type="text" name="stream" id="stream" class="form-control" value="{{ old('stream', optional($department)->stream) }}" placeholder="e.g 30ae7639-646f-45ff-bee1-9ff172682c0b" tabindex="7" autocomplete="off" aria-describedby="helpBlockStream">
						...
						<div class="custom-controls-stacked">
							@foreach ($statuses as $status)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start status_id-{{ $status->id }}">
									<input type="radio" name="status_id" id="status_id-{{ str_slug($status->title) }}" class="custom-control-input" value="{{ $status->id }}" tabindex="8" aria-describedby="helpBlockStatusId" {{ ($department->status_id == $status->id || old('status_id') == $status->id) ? 'checked' : '' }}>
