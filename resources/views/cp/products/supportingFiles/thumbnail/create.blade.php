...
@section('title', 'Add Thumbnail - Supporting File - Product - Products - '.config('app.name'))
@section('description', 'Add Thumbnail - Supporting File - Product - Products - '.config('app.name'))
@section('keywords', 'Add, Supporting, File, Thumbnail, Product, Products, '.config('app.name'))
...
@section('formButtons')
	<div class="form-buttons">
		@if ($currentUser->hasPermission('view_products'))
			<a href="/cp/products" title="Cancel" class="btn btn-link text-secondary cancel-button" title="Cancel">Cancel</a>
		@endif
...
@section('content')
		...
		<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
			...
			<div class="content padding bg-white">
				<form name="uploadSupportingFileThumbnail" id="uploadSupportingFileThumbnail" class="uploadSupportingFileThumbnail" role="form" method="POST" action="/cp/products/{{ $id }}/supporting-files/{{ $supportingFileId }}/thumbnail" enctype="multipart/form-data">
					{{ csrf_field() }}
					@yield('formButtons')
