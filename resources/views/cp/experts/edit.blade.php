...
@section('title', 'Edit Expert - Experts - '.config('app.name'))
@section('description', 'Edit Expert - Experts - '.config('app.name'))
@section('keywords', 'Edit, Expert, Experts, '.config('app.name'))
...
@section('formButtons')
	<div class="form-buttons">
		...
		@endif
		<button type="submit" name="submit_edit_expert" id="submit_edit_expert" class="pull-right btn btn-primary" title="Save Changes"><i class="icon fa fa-check-circle" aria-hidden="true"></i>Save Changes</button>
	</div>
@endsection
...
@section('content')
		...
		<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
			...
			<div class="content padding bg-white">
				<form name="editExpert" id="editExpert" class="editExpert" role="form" method="POST" action="/cp/experts/{{ $expert->id }}">
					{{ csrf_field() }}
					{{ method_field('PUT') }}
					...
					<div class="form-group">
						...
						<input type="text" name="full_name" id="full_name" class="form-control" value="{{ old('full_name', optional($expert)->full_name) }}" placeholder="e.g Test Full Name" tabindex="1" autocomplete="off" aria-describedby="helpBlockFullName" required autofocus>
					...
					<div class="form-group">
						...
						<input type="email" name="email" id="email" class="form-control" value="{{ old('email', optional($expert)->email) }}" placeholder="e.g Test Email" tabindex="2" autocomplete="off" aria-describedby="helpBlockEmailAddress" required>
						@if ($errors->has('email'))
					...
					<div class="form-group">
						...
						<input type="text" name="position" id="position" class="form-control" value="{{ old('position', optional($expert)->position) }}" placeholder="e.g Test Position" tabindex="3" autocomplete="off" aria-describedby="helpBlockPosition" required>
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="text" name="image" id="image" class="form-control" autocomplete="off" placeholder="e.g Test Mugshot" value="{{ old('image', optional($expert)->image->path) }}" tabindex="4" autocomplete="off" aria-describedby="helpBlockMugshot">
							<div class="input-group-append">
								<a href="javascript:void(0);" title="Select Asset" rel="nofollow" data-toggle="modal" data-target="#image-browse-modal" data-field_id="image" data-value="{{ old('image', optional($expert)->image->url) }}" class="btn btn-secondary">Select Asset</a>
						...
						<div class="custom-controls-stacked">
							@foreach ($statuses as $status)
								<div class="custom-control custom-radio d-flex h-100 justify-content-start status_id-{{ $status->id }}">
									<input type="radio" name="status_id" id="status_id-{{ str_slug($status->title) }}" class="custom-control-input" value="{{ $status->id }}" tabindex="5" aria-describedby="helpBlockStatusId" {{ ($expert->status_id == $status->id || old('status_id') == $status->id) ? 'checked' : '' }}>
					...
					<div class="form-group">
						...
						@else
							@php ($categoryIds = $expert->categories->pluck('id')->toArray())
						@endif
